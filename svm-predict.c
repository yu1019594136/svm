...
RESULT_PREDICT_ACCURACY result_predict_accuracy;//zy,用于保存预测正确率
...
//	}
    ...
    predict_probability = para_svm_predict->predict_probability;

    if(para_svm_predict->quiet_mode)
        info = &print_null;
...
//	}
    ...
    /* 获取测试样本 */
    if((input = fopen(para_svm_predict->test_file, "r")) == NULL)
	{
    ...
    /* 打开输出结果文件 */
    if((output = fopen(para_svm_predict->output_file, "w")) == NULL)
	{
    ...
    /* 获取模型文件 */
    if((model=svm_load_model(para_svm_predict->model_file)) == 0)
	{
